            ...
            <div class="relative" x-data="{ open: false }">
                <button aria-haspopup="true" aria-expanded="false" aria-label="User menu"
                    ...
                    <div class="w-10 h-10 rounded-full bg-gray-300 flex items-center justify-center text-gray-600 font-bold">
                        <?php
                        // Corrected: use prepared statement for getting username
...
<?php
// Mapping community to image URLs
$community_links = [
    "Funny" => "s/funny.php", // Corrected relative path for s/funny.php
    "Gaming" => "s/gaming.php",
    "Movies" => "s/movies.php",
...
// Mapping community to image URLs for thumbnails/icons
$community_images = [
...
while ($post = mysqli_fetch_assoc($result)) {
    // Determine the image to display for the post content itself
    // Use post.image_url if available, otherwise fallback to community image
    $display_img_url = !empty($post['image_url']) ? htmlspecialchars($post['image_url']) : ''; // Use empty string if no image

    // Determine the image for the community icon next to post info
    $community_icon_url = $community_images[$post['community']] ?? 'default_image_url.jpg';
        ...
        <div class="flex p-3">
            ...
            <div class="flex-1">
                <div class="flex items-center space-x-2 text-xs text-gray-500 mb-1">
                    <img alt="Community icon" class="w-5 h-5 rounded-full" src="' . htmlspecialchars($community_icon_url) . '" />
    ...
    if (!empty($display_img_url)) {
        echo '<img class="mt-2 rounded-md max-h-96 w-auto object-cover" src="' . $display_img_url . '" alt="Post Image" />';
    }
