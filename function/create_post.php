...
if (isset($_FILES['image-upload']) && $_FILES['image-upload']['error'] === UPLOAD_ERR_OK) {
    $file_tmp_name = $_FILES['image-upload']['tmp_name'];
    ...
    if (!in_array($file_ext, $allowed_extensions)) {
        die('Format gambar tidak diizinkan. Hanya JPG, JPEG, PNG, dan GIF yang diperbolehkan.');
    }

    if ($file_size > $max_file_size) {
        die('Ukuran gambar terlalu besar. Maksimal 5MB.');
    }

    // Buat nama file unik
    $new_file_name = uniqid('img_', true) . '.' . $file_ext;
    $upload_dir = __DIR__ . '/../uploads/post_images/'; // Sesuaikan path folder uploads
    ...
    $relative_image_path = '/uploads/post_images/' . $new_file_name; // Path untuk disimpan di DB
...
} elseif (isset($_POST['image-url']) && !empty(trim($_POST['image-url']))) {
    // Fallback to external URL if no file uploaded but URL is provided
    $image_url_for_db = trim($_POST['image-url']);
}
...
// Insert post tanpa location dulu, gunakan $image_url_for_db
$stmt = mysqli_prepare($link, "INSERT INTO post (owner, community, title, body, image_url) VALUES (?, ?, ?, ?, ?)");
mysqli_stmt_bind_param($stmt, "issss", $owner_id, $community, $title, $body, $image_url_for_db);
...
if (!empty($image_url_for_db)) {
    $image_url_escaped = htmlspecialchars($image_url_for_db);
    $image_html = <<<HTML
        <img class="rounded-md mb-4 w-full object-cover" src="{$image_url_escaped}" alt="Post Image" />
HTML;
}

// Dapatkan URL lengkap untuk post yang baru dibuat
// Ini akan menjadi URL yang akan disalin
$full_post_url = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]$relative_path";
...
<html lang="en">
  <head>
    ...
    <title>{$safe_title_html}</title> <script src="https://cdn.tailwindcss.com"></script>
